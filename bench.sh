...
hyperfine --warmup=10 \
  ...
  "sqlite3 :memory: '.load target/release/libfastrand0' 'select count(fastrand_int64()) from generate_series(1, $END);'" \
  "sqlite3 :memory: '.load target/release/libfastrand0' 'select count(fastrand_i64()) from generate_series(1, $END);'"
